...
description: Use when writing or reviewing Playwright tests and browser automation, debugging flaky CDP/browser behavior, selecting locators and wait strategies, capturing network responses, handling authentication or multi-page flows, or configuring browser tests in CI. For this repository, apply it to AdsPower-attached Playwright/CDP collection and future Seller Center integration tests.
---
...
For this repository, preserve the existing AdsPower CDP attachment and `playwright-core` dependency. Do not introduce a second browser runtime or persist cookies, tokens, storage state, or buyer PII.
