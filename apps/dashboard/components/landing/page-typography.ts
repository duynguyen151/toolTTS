...
export type PageTypographyProps = {
  ...
  headingLetterSpacing?: number | undefined;
};
...
export type LandingPageCustomization = {
   ...
   */
  inlineStyles?: readonly PageInlineStyleOverride[] | undefined;
};
...
function hexToHsl(hex: string): Hsl {
  const rVal = Number.parseInt(hex.slice(1, 3), 16) / 255;
  const gVal = Number.parseInt(hex.slice(3, 5), 16) / 255;
  const bVal = Number.parseInt(hex.slice(5, 7), 16) / 255;
  const red = Number.isFinite(rVal) ? rVal : 0;
  const green = Number.isFinite(gVal) ? gVal : 0;
  const blue = Number.isFinite(bVal) ? bVal : 0;
...
function hslToRgb({ h, s, l }: Hsl) {
  ...
  const [r, g, b] = h < 60 ? [c, x, 0]
    ...
    : h < 300 ? [x, 0, c]
    : [c, 0, x];
  return [(r ?? 0) + m, (g ?? 0) + m, (b ?? 0) + m].map((channel) => Math.round(clamp01(channel) * 255));
}
...
function selectFont(value: string | undefined, options: readonly PageFont[]): PageFont {
  const found = options.find((option) => option.value === value) ?? options[0];
  if (!found) {
    return {
      value: "system",
      label: "System",
  ...
  return useMemo(() => {
    ...
    const retoneRgba = (color: string) => {
      ...
      const hex = `#${[match[1], match[2], match[3]]
        ...
        .join("")}`;
      const retoned = retone(hex);
      const red = Number.parseInt(retoned.slice(1, 3), 16) || 0;
      const green = Number.parseInt(retoned.slice(3, 5), 16) || 0;
      const blue = Number.parseInt(retoned.slice(5, 7), 16) || 0;
